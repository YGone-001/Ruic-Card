...
void main() {
  ...
  // A quiet ink wash beneath the fixed typography preserves legibility over
  // the detailed brickwork without turning the card furniture into a solid box.
  float topInk = 1. - smoothstep(.025,.22,uv.y);
  float bottomInk = smoothstep(.76,.975,uv.y);
  float inkField = max(topInk,bottomInk);
  col = mix(col,col*.73+vec3(.025,.032,.028),inkField*.26);
...
function backTexture() {
  ...
  ctx.fillText((config.backMark || config.title || "A").slice(0, 1), 512, 846);
  ctx.font = "24px Arial";
...
async function readCardConfig(cardId) {
  const response = await fetch(CARD_CONFIG_URLS[cardId]);
...
function syncCardPicker() {
  document.querySelectorAll("[data-card]").forEach((button) => {
    button.setAttribute("aria-pressed", String(button.dataset.card === activeCardId));
  });
}
async function loadLayerTextures(cardConfig) {
  const loader = new THREE.TextureLoader();
  const primary = await Promise.all(
    ["subject", "background", "text"].map((name) => loader.loadAsync(cardConfig.assets[name])),
  );
  const line = cardConfig.assets.lineart
    ? await loader.loadAsync(cardConfig.assets.lineart)
    : new THREE.DataTexture(new Uint8Array([255, 255, 255, 255]), 1, 1);
  line.needsUpdate = true;
  const hasFx = !!cardConfig.assets.effects;
  const effects = hasFx
    ? await loader.loadAsync(cardConfig.assets.effects)
    : new THREE.DataTexture(new Uint8Array([0, 0, 0, 0]), 1, 1);
  effects.colorSpace = THREE.NoColorSpace;
  if (!hasFx) effects.needsUpdate = true;
  const all = [...primary, line, effects];
  all.forEach((texture) => {
    texture.colorSpace = THREE.NoColorSpace;
...
function applyCardUniforms(layers) {
  const p = config.parameters || {};
  const imageAspect = layers.subject.image.width / layers.subject.image.height;
  const fit =
    config.artworkFit ||
    (config.sourceMode === "reference"
      ...
      : [1, 1]);
  uniforms.tSubject.value = layers.subject;
  uniforms.tBackground.value = layers.background;
  uniforms.tText.value = layers.text;
  uniforms.tLine.value = layers.line;
  uniforms.tEffects.value = layers.effects;
  const previousBack = uniforms.tBack.value;
  uniforms.tBack.value = backTexture();
  previousBack?.dispose();
  uniforms.uFit.value.set(...fit);
  uniforms.uFoil.value = p.foil ?? 0.52;
  uniforms.uScale.value = p.subjectScale ?? 1;
  uniforms.uDepth.value = p.subjectDepth ?? 0.32;
  uniforms.uBgDepth.value = p.backgroundDepth ?? -0.18;
  uniforms.uSafeScale.value = config.safeArea?.scale ?? 1;
  uniforms.uSafeOffset.value.set(
    config.safeArea?.offset?.[0] ?? 0,
    1 - (config.safeArea?.scale ?? 1) - (config.safeArea?.offset?.[1] ?? 0),
  );
  uniforms.uFxDepth.value = p.effectsDepth ?? 0.14;
  uniforms.uHasFx.value = layers.hasFx ? 1 : 0;
  uniforms.uHasLine.value = config.assets.lineart ? 1 : 0;
  uniforms.uRelief.value = config.sourceMode === "relief" ? 1 : 0;
}
function mountModel(modelScene) {
  if (root) scene.remove(root);
  Object.values(reliefLayers).forEach((meshes) => (meshes.length = 0));
  ...
  if (config.sourceMode === "relief" && !reliefLayers.subject.length) {
    throw Error("缺少独立人物层，请重新生成模型");
  }
  root.rotation.set(targetX, targetY, 0);
}
function setupCardPicker() {
  document.querySelectorAll("[data-card]").forEach((button) => {
    button.onclick = () => switchCard(button.dataset.card);
  });
  syncCardPicker();
}
async function switchCard(cardId) {
  if (!CARD_CONFIG_URLS[cardId] || cardId === activeCardId || switchingCard) return;
  // The CSS-3D fallback creates DOM layers once. Reload it onto the selected
  // config rather than risking a stale texture stack when WebGL is unavailable.
  ...
  switchingCard = true;
  document.querySelectorAll("[data-card]").forEach((button) => (button.disabled = true));
  try {
    const nextConfig = await readCardConfig(cardId);
    const [nextTextures, nextGltf] = await Promise.all([
      loadLayerTextures(nextConfig),
      new GLTFLoader().loadAsync(nextConfig.assets.model),
    ]);
    const previousTextures = activeTextures;
    config = nextConfig;
    activeCardId = cardId;
    activeTextures = nextTextures.all;
    updateCardDetails();
    applyCardUniforms(nextTextures);
    mountModel(nextGltf.scene);
    renderer.setClearColor(config.appearance?.background || "#fafafa", 1);
    syncCardPicker();
    reset();
    setAuto(!media.matches);
    if (window.__holo) {
      window.__holo.config = config;
      window.__holo.root = root;
      window.__holo.modelSource = config.assets.model;
      window.__holo.layers = reliefLayers;
    }
    previousTextures.forEach((texture) => texture.dispose());
    notice(`已装裱：${config.title}`);
  } catch (error) {
    console.error("[holo-card] card switch failed", error);
    notice("切换失败，请检查所选卡片资源");
  } finally {
    switchingCard = false;
    document.querySelectorAll("[data-card]").forEach((button) => (button.disabled = false));
  }
}
async function init() {
  refreshIcons();
  config = await readCardConfig(activeCardId);
  updateCardDetails();
  setupCardPicker();
  ...
  const layers = await loadLayerTextures(config);
  activeTextures = layers.all;
  const p = config.parameters || {};
  const imageAspect = layers.subject.image.width / layers.subject.image.height;
  const fit =
    config.artworkFit ||
    (config.sourceMode === "reference"
      ...
      : [1, 1]);
  uniforms = {
    tSubject: { value: layers.subject },
    tBackground: { value: layers.background },
    tText: { value: layers.text },
    tLine: { value: layers.line },
    tEffects: { value: layers.effects },
...
function fallback3D(error) {
  ...
  backMark.textContent = (config.backMark || config.title || "白").slice(0, 1);
